...
$VARIABLES{'configfile'} = 'nikto_files/nikto.conf';    ### Change if it's having trouble finding it
...
$CLI{'host'}	= $target[0];
$CLI{'ports'}	= $target[2];
$CLI{'ssl'}		= $target[1] eq 'https' ? 1 : 0;

setup_dirs();
require 'nikto_files/plugins/nikto_core.plugin';
nprint("T:" . localtime($COUNTERS{'scan_start'}) . ": Starting", "d");
require 'nikto_files/plugins/nikto_single.plugin';
require 'nikto_files/plugins/LW2.pm';
...
sub setup_dirs {
    my $CURRENTDIR = $0;
    chomp($CURRENTDIR);
    $CURRENTDIR = 'C:/GTTAScriptExtra/nikto_vulnerab';
    ...
    unless (defined $CONFIGFILE{'PLUGINDIR'}) {
        $CONFIGFILE{'PLUGINDIR'} = 'C:/GTTAScriptExtra/nikto_vulnerab/plugins';
    }
    unless (defined $CONFIGFILE{'TEMPLATEDIR'}) {
        $CONFIGFILE{'TEMPLATEDIR'} = 'C:/GTTAScriptExtra/nikto_vulnerab/templates';
    }
    unless (defined $CONFIGFILE{'DOCUMENTDIR'}) {
        $CONFIGFILE{'DOCUMENTDIR'} = 'C:/GTTAScriptExtra/nikto_vulnerab/docs';
    }
    return;
}
