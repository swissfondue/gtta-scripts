...
site     = None         # URL to tree/scan
domains  = []           # whitelist of domains to scan, with a "." infront, it matches all subdomains as well
pagetype = None         # type of code used on the site being scanned ("php" or "asp")

# log locations, all logs are stored at [the current directory]/logs/LOGFILE
usecookies=False                     # whether or not to send a cookie header value each time (True or False)
...
def removeurls(body): return re.sub("http://([^ \"\']+)","",body)
...
def getpage(url,dheaders=1,redir=0,realpage=0,poststring="",exceptions=0):
        ...
        global usecookies,urllogfile,ignorefileext, check_stop, socket_timeout
        ...
        fileext=re.sub(".*(http\://[^/]*/).*","\\1",url)
        if url==fileext: fileext="None"
        else: fileext=re.sub("^.*\/[^/]*\.([^\&\#\?\/]*)[^/]*$","\\1",url)
...
#                                       if url[len(url)-2:]=="" and locpage[len(locpage)-4:]=="": break
                                        ...
                                        if locpage!=url:
                                                redir=0
                                                if pagename(re.sub("\\\\(\"|\')","\\1",locpage))==pagename(url):
                                                        #print "QUOTE REDIR"
                                                        return
                                        #print "OLD:",url
...
def decapitate(body):
        ...
        global decapitation
        if decapitation==True: body=re.sub("<head>(.+?)</head>","",body)
...
def exploit(url,qry,namval,explt,post,poststring="",randstr=""):
        ...
        def removeexplt(body,explt):
                ...
                try:
                        explt=str(int(explt))
                        body=re.sub("(?<![0-9])"+explt+"(?![0-9])","",body)
        ...
        expltqry=re.sub(namval+"[^\&]*(?=\&|$)",namval+explt,qry)
...
def parsebody(body):
        ...
        if pagetype=="php":
                body=re.sub("(?i)\<input type=\"hidden\" name=\"phpsessid\" value=\"([a-z0-9]{32})\"( /|)\>","",body)
        body=re.sub(" alt=(\"|\')([^\\1\>]*)\\1","",body)
...
#               if name=="xss": return
                ...
                if xssscan:
                        ...
                        if checklevel<=2:
                                ...
                                if post==0: body=getpage(theurl+re.sub(namval+"[^\&]*(?=\&|$)",namval+xssplt,qry),poststring=poststring)
                                else: body=getpage(theurl,poststring=re.sub(namval+"[^\&]*(?=\&|$)",namval+xssplt,qry))
...
def treepages(url,level):
        ...
        global treeglob,urlfields,postfields,treedurls,levels,server,vulnlogfile,scanlimit,ignorefileext, url_limit
        #print ">>>>>>>>",level,"<<<<<<<<"
        ...
        for i in range(len(bodyarr)):
                ...
                poststring=""
                inputarr=re.findall("<input([^>]*)>", frmbody, re.IGNORECASE)
                ...
                if re.search("method=([\'\"]|)post([\'\"]|)",frmsect[:frmsect.find(">")].lower())==None:
        ...
        urlarr=re.sub("(?s)(?i)(.+?)((src|href)=|location([\ ]*)=([\ ]*)|window\.open\()"+urlreg+"\\6","\\7|ZZaaXXaaZZ|",body).split("|ZZaaXXaaZZ|")
        del urlarr[len(urlarr)-1]
        urlarr.append(re.sub("(?s)(?i)(.+?)(src|href)="+urlreg+"\\3","\\4|ZZaaXXaaZZ|",body).split("|ZZaaXXaaZZ|"))
        del urlarr[len(urlarr)-1]
...
def detect(p_site, p_domains, p_pagetype, p_cookies, p_url_limit, p_check_stop, p_socket_timeout):
        global treedurls, reportvar, site, domains, pagetype, cookies, usecookies, comnt, sessstr, ignorefileext, check_stop, socket_timeout, url_limit
        ...
        url_limit      = p_url_limit
        check_stop     = p_check_stop
        socket_timeout = p_socket_timeout
        ...
        if pagetype=="asp": comnt="%2d%2d"
                ...
                output.append("Unknown Page Type: "+pagetype.upper())
                output.append("Using Default Database Comment: --")
                comnt="%2d%2d"
        ...
        if pagetype=="asp": sessstr+="aspsessionid([a-z]{8})=([a-z]{8})"
                ...
                output.append("Using Default Session Syntax: --")
        ...
        try:
            ...
            output.append('URL limit exceeded.')
                        ...
                        if not listempty(reporturl):
                                ...
                                if not listempty(reporturl[0]):
                                        if not listempty(reporturl[0][0]) or not listempty(reporturl[0][1]):
                                                ...
                                                output_str = indent+"GET: "

                                                if not listempty(reporturl[0][0]):
                                                        output_str += "SQLI - "+str(reporturl[0][0])
                                                        if not listempty(reporturl[0][1]):
                                                                output_str += " | "
                                                if not listempty(reporturl[0][1]):
                                                        output_str += "XSS - "+str(reporturl[0][1])
                                ...
                                if not listempty(reporturl[1]):
                                        if not listempty(reporturl[1][0]) or not listempty(reporturl[1][1]):
                                                ...
                                                output_str = indent+"POST: "
                                                if not listempty(reporturl[1][0]):
                                                        output_str += "SQLI - "+str(reporturl[1][0])
                                                        if not listempty(reporturl[1][1]):
                                                                output_str += " | "
                                                if not listempty(reporturl[1][1]):
                                                        output_str += "XSS - "+str(reporturl[1][1])
        ...
        output.append('Scanned %i URLs.' % len(treedurls))
        output.append("%.2f seconds elapsed." % timeelapsed)
