...
sub cmsPlugins{ # Plugin databases provided by: Chris Sullo from cirt.net
	...
	if($cmsPlugins =~ /dp/i){
		push(@cmsPluginDBlist, 'files/drupal_plugins.db');
	}

	if($cmsPlugins =~ /jm/i){
		push(@cmsPluginDBlist, 'files/joomla_plugins.db');
	}

	if($cmsPlugins =~ /wp/i){
		push(@cmsPluginDBlist, 'files/wp_plugins.db');
	}

	if($cmsPlugins =~ /all/i ){
		@cmsPluginDBlist = ('files/drupal_plugins.db', 
		'files/joomla_plugins.db', 
		'files/wp_plugins.db');
	}
...
sub FilesAndDirsGoodies{ # databases provided by: raft team
	...
	my @FilesAndDirsDBlist = ('files/raft-small-files.db',
	'files/raft-small-directories.db',);
...
sub webServices{ # as of v 1.2.7 it's acually worth the time typing "-Ws" to use it! HORAYYY
	...
	open(webServicesDB, "+<", 'files/web-services.db');
...
sub faviconMD5{ # thanks to OWASP
	...
	if($favicon->is_success){
		...
		open(faviconMD5DB, "+<", 'files/favicon.db');
