...
my $target = $ARGV[0];
my $big_file = $ARGV[1];
my $start_harvester = $ARGV[2];
my $harvester_path = $ARGV[3];
...
if ($start_harvester) {
	my @theHarvester_output = `python $harvester_path/theHarvester_domains.py -b all -t -d $target`;
