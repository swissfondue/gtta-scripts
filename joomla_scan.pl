...
sub scanbugs {
	...
	print OUTFILE 'search for possibles bugs',"($type)\n";
	open(DAT, "joomla_scan_files/db/" . $file) || die("File " . $file . " not found. Try --force-update\n");
...
sub scantips {
	...
	# print "Scanning " . $file . " ";
	print OUTFILE 'search for unprotected files',"($file)\n";
	open(DAT, "joomla_scan_files/db/" . $file) || die("File " . $file . " not found. Try --force-update\n");
...
sub search_meta {
	...
	print OUTFILE "Searching for copyright of version " . $check . "\n";
...
sub scancomponents {
	...
	print OUTFILE "Searching for components ";
...
sub scanfirewall {
	print OUTFILE "Searching for firewall\n";
...
sub checkdatafiles {
	for (my $i = 0; $i <= $#data; $i++) {
		open(DAT, "joomla_scan_files/db/" . $data[$i]) || die("File " . $data[$i] . " not found. Try --force-update\n");
		close(DAT);
	}
...
sub download {
	...
	if ($update_path eq $update_path_db) {
		$file = "joomla_scan_files/db/" . $file;
	}
...
sub update {
	open(DAT, "joomla_scan_files/db/" . $dbversion) || die("File " . $dbversion . " not found. Try --force-update\n");
	my @data = <DAT>;
	close(DAT);
	...
	if ($joomlascanversion ne $lastProgramVersion) {
		print OUTFILE "Current version: " . $joomlascanversion . " - Last version: " . $lastProgramVersion . "\n";
		updateprogram();
		$act = 1;
	}

	if ($currentVersion ne $lastDbVersion) {
		print OUTFILE "Current version: " . $currentVersion . " - Last version: " . $lastDbVersion . "\n";
		updatedatabase();
		$act = 1;
	}

	if ($act eq 0) {
		print OUTFILE "No new updates available\n";
	}
...
# 	logo();
	...
	if ($webserver ne "") {
		print OUTFILE "Running on " . $webserver . "\n\n";
	}

	if ($all eq 1) {
		if ($firewall ne "") { print OUTFILE "Firewall: " . $firewall . "\n\n"; }
		else { print OUTFILE "No firewall detected\n\n"; }
	}
	...
	if ($all eq 1) {
		...
		print OUTFILE $jversion . "\n\n";
	}

	if ($all eq 1) {
		print OUTFILE "Security tips:\n";
		print OUTFILE "=============\n";

		for (my $i = 0; $i <= $#sVulnerability; $i++) {
			print OUTFILE "Info: " . $sVulnerability[$i] . "\n";
			print OUTFILE "Versions affected: " . $sVersion[$i] . "\n";
			print OUTFILE "Files affected: " . $sFile[$i] . "\n";
			print OUTFILE "Detail: " . $sExploit[$i] . "\n";

			if ($sUrlExploit[$i] =~ m/ /) {
				print OUTFILE "More info: ";
				...
				print OUTFILE "More info: " . $sUrlExploit[$i] . "\n";
			}
	...
	if ($all eq 1) {
		print OUTFILE "Possible vulnerabilities in core:\n";
		print OUTFILE "================================\n";

		for (my $i = 0; $i <= $#bVulnerability; $i++) {
			if ($bType[$i] eq "Core") {
				print OUTFILE "Possible vulnerability: " . $bVulnerability[$i] . "\n";
				print OUTFILE "Versions affected: " . $bVersion[$i] . "\n";
				print OUTFILE "Detail: " . $bExploit[$i] . "\n";
				...
				if ($bUrlExploit[$i] =~ m/ /) {
					...
					print OUTFILE "More info: " . $bUrlExploit[$i] . "\n";
				}
			}
		}
	...
	if ($all eq 1) {
		print OUTFILE "Possible vulnerabilities in components:\n";
		print OUTFILE "======================================\n";
...
#
# v1.2 - 2010-09-06
# - Correct some errors checking Joomla! version
# - Added web server fingerprinting
# - Added joomlascan version option
# - Correct some update errors
#
# v1.1 - 2010-09-06
